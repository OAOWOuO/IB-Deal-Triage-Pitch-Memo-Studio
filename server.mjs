...
async function runProductSelfTest() {
  ...
  return {
    ...
    agent: {
      ...
      workflowStep: "00. Product readiness self-test",
      usedWhen: "Before or during deal review when the team needs to verify the workspace itself.",
      inputs: ["Static shell", "frontend code", "server code", "package scripts", "SEC configuration"],
      output: "Pass/gap product readiness report.",
      decisionUse: "Confirms the studio is safe to use as a deal workpaper before trusting memo outputs.",
      controlGate: "Product self-test passes",
  ...
  return [
    {
      ...
      workflowStep: "01. Intake and identity resolution",
      usedWhen: "Runs immediately after the target ticker or CIK resolves.",
      inputs: ["SEC submissions profile", "recent filing metadata", "public quote availability"],
      output: "Target identity, filer status, latest filing context, and mandate gaps.",
      decisionUse: "Confirms the memo is anchored to the correct legal issuer before any deal readout.",
      controlGate: "SEC identity resolved",
    ...
    },
    {
      ...
      workflowStep: "02. Deal-party and buyer lens",
      usedWhen: "Runs after the optional acquirer / buyer field is submitted.",
      inputs: acquirer ? ["Acquirer SEC profile", "acquirer XBRL facts", "acquirer quote data"] : ["Target packet", "empty acquirer field"],
      output: "Buyer-side capacity screen or a clear target-only designation.",
      decisionUse: "Separates target triage from buyer-specific strategic fit, financing, and capacity analysis.",
      controlGate: "Acquirer lens selected or intentionally open",
    ...
    },
    {
      ...
      workflowStep: "03. Reported financial extraction",
      usedWhen: "Runs after SEC companyfacts are loaded for the target.",
      inputs: ["SEC companyfacts XBRL tags", "filing dates", "accepted fiscal-period filters"],
      output: "Revenue, profitability, cash flow, balance sheet, and missing-tag flags.",
      decisionUse: "Determines which reported metrics can be cited and which diligence items remain open.",
      controlGate: "Reported financial facts traceable",
    ...
    },
    {
      ...
      workflowStep: "04. Market and comps screen",
      usedWhen: "Runs after target quote, share count, debt/cash facts, and peer universe are available.",
      inputs: ["public quote feed", "SEC share count", "cash and debt facts", peerMode === "explicit" ? "banker-approved peers" : "suggested SEC directory screen"],
      output: "Market cap, enterprise value, trading multiples, peer medians, and comp approval gaps.",
      decisionUse: "Supports relative valuation discussion only when inputs are source-backed and peer scope is approved.",
      controlGate: "Peer universe available and approved",
    ...
    },
    {
      ...
      workflowStep: "05. Capital structure screen",
      usedWhen: "Runs after cash, debt, equity, and EBITDA availability are checked.",
      inputs: ["cash facts", "debt facts", "equity facts", "EBITDA derivation status"],
      output: "Liquidity, leverage availability, and financing diligence gaps.",
      decisionUse: "Flags whether financing capacity discussion is evidence-backed or still needs debt schedule work.",
      controlGate: "Balance sheet facts traceable",
    ...
    },
    {
      ...
      workflowStep: "06. Disclosure and diligence escalation",
      usedWhen: "Runs after filing metadata, risk flags, and source limitations are assembled.",
      inputs: ["recent filing list", "risk register", "data limitations", "source provenance"],
      output: "Disclosure gaps, diligence escalations, and review constraints.",
      decisionUse: "Prevents the memo from presenting unresolved filing or diligence limitations as conclusions.",
    ...
    },
    {
      ...
      workflowStep: "07. MD synthesis and memo readiness",
      usedWhen: "Runs after all analyst workstreams and harness gates are available.",
      inputs: ["agent outputs", "harness gates", "quality score", "banker notes"],
      output: "Committee-ready disposition, unresolved blockers, and memo-use boundary.",
      decisionUse: "Tells the deal team whether the output is triage-only, internal review, or ready for broader circulation.",
      controlGate: "Harness readiness disposition",
